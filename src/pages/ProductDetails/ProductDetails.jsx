...
const ProductQuickDetails = () => {
  ...
  const {isLoading, data: product} = useQuery([
    'product', productId
  ], () => fetch(`https://nexiq-server.vercel.app/product/${productId}`).then(res => res.json()))
  if (isLoading) {
    return <Loading/>
  }
  const {
      ...
      <div >
        <div className='grid lg:grid-cols-2'>
          ...
          <div className='flex items-center'>
            <div>
              <p className='text-4xl font-bold text-primary'>{name}
              </p>
              <p className='text-secondary text-2xl font-bold py-3'>${price}
              </p>
              ...
              <button
                ...
                className='bg-primary shadow-md shadow-secondary/50 text-base-100 px-10 py-2 rounded my-5'>Order Now</button>
